    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-[#1e2124] border border-white/10 rounded-3xl shadow-2xl w-full max-w-md mx-4 p-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-400 text-sm">Upload foto struk untuk mengisi transaksi secara otomatis.</p>
        </div>
        ...
        {error && (
          <p className="text-red-400 text-sm mb-4 bg-red-500/10 border border-red-500/20 px-4 py-3 rounded-xl">{error}</p>
        )}

        {/* Actions */}
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 bg-white/8 hover:bg-white/12 text-gray-300 font-semibold py-3.5 rounded-2xl text-sm transition-colors">
            Batal
          </button>
          <button
            onClick={handleScan}
...
export default function Dashboard() {
      ...
      try {
        ...
        if (dashboardData.summary) {
          ...
          if (dashboardData.summary.chart) {
            const formattedChart = Object.keys(dashboardData.summary.chart).map((key) => ({
              name: key,
        ...
        setError("Gagal memuat data. Pastikan server berjalan dan Anda sudah login.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <MainLayout isLoading={isLoading}>
      {showScanModal && <ScanReceiptModal onClose={() => setShowScanModal(false)} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-2 bg-gradient-to-br from-[#065f46] to-[#0f172a] rounded-3xl p-8 flex flex-col md:flex-row justify-between items-center relative overflow-hidden shadow-lg border border-emerald-900/30">
          <div className="z-10 w-full mb-6 md:mb-0">
            <p className="text-xs font-bold text-emerald-100/70 mb-2 uppercase tracking-widest">TOTAL NET WORTH</p>
            <h3 className="text-5xl font-extrabold text-white mb-3">{formatRupiah(summary.net_balance)}</h3>
            <span className="inline-block bg-emerald-800/60 text-emerald-100 text-xs px-2.5 py-1 rounded-md font-medium mb-8">ALL TIME</span>
            ...
            <div className="flex flex-wrap gap-8 border-b border-emerald-800/50 pb-6 mb-6">
              <div>
                ...
                <p className="text-lg font-bold text-white">{formatRupiah(summary.total_income)}</p>
              </div>
              <div>
                <p className="text-xs text-emerald-200/60 mb-1">Total Expenses</p>
                <p className="text-lg font-bold text-white">{formatRupiah(summary.total_expense)}</p>
              </div>
            </div>

            <div>
              <p className="text-xs text-emerald-200/60 mb-1">Top Category Expense</p>
              <p className="text-sm font-semibold text-orange-300">{summary.top_category}</p>
            </div>
          </div>
          ...
          <div className="z-10 w-80 h-72 shrink-0 flex flex-col items-center">
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={chartData} innerRadius={70} outerRadius={90} paddingAngle={8} dataKey="value"
                    stroke="none" cornerRadius={6} animationBegin={0} animationDuration={1500}>
                  ...
                  <Tooltip
                    contentStyle={{ backgroundColor: "rgba(15, 23, 42, 0.9)", border: "1px solid rgba(16, 185, 129, 0.2)", borderRadius: "12px", color: "#fff", padding: "10px" }}
              ...
              <div className="w-48 h-48 rounded-full border-2 border-dashed border-emerald-500/20 flex flex-col items-center justify-center bg-emerald-500/5">
                <span className="text-emerald-500/40 text-xs font-medium">No Data Yet</span>
              </div>
            )}
            <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 mt-2">
              {chartData.slice(0, 3).map((entry, index) => (
                <div key={index} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-emerald-100/60 uppercase tracking-tighter">{entry.name}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 h-full">
            ...
            >
              <div className="bg-gray-500/20 p-4 rounded-full mb-3 border border-gray-500/30 group-hover:border-emerald-500/40 group-hover:bg-emerald-500/10 transition-all">
                <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 text-gray-300 group-hover:text-emerald-400 transition-colors">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 8V6a2 2 0 012-2h2M3 16v2a2 2 0 002 2h2m10-14h2a2 2 0 012 2v2m-2 10h-2a2 2 0 01-2-2v-2M8 12h8m-4-4v8" />
                </svg>
              </div>
